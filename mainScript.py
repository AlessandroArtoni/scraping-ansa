        ...
        while count < 10:
            ...
            if len(title) > 1:
                try:
                    ...
                    endDay = date.find("-")
                    endMonth = date.find("-", endDay+1)
                    day = date[:endDay]
                    month = date[endDay+1:endMonth]
                    year = date[endMonth+1:]
                    date = year + "/" + month + "/" + day
                    date = date.replace(" ", "")
                    ...
                    try:
                        with connection.cursor() as cursor:
                            query = "INSERT INTO articles_finanza_com (date, newspaper, section, title, body, company, author, tagged_companies) VALUES (str_to_date(%s,'%%Y/%%m/%%d'), %s, %s, %s, %s, %s, %s, %s)"
                            cursor.execute(query,
                                           [date, "finanza.com", "economy", title, bodyArticle, nameCompany, nomeAutore,
                                            linkedCompanies])
                            connection.commit()
